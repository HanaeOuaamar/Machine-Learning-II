...
stateDiagram-v2
    [*] --> Initialisation
    Initialisation --> Exploration: Choisir action (ε-greedy)
    Exploration --> Execution: Appliquer action
    Execution --> MiseAJour: Mettre à jour Q-table
    MiseAJour --> Verification: État terminal ?
    Verification --> [*]: Oui
    Verification --> Exploration: Non
```
  ...
  ##### 📊 Schéma du fonctionnement des feux de circulation

```mermaid
graph TD;
    Start --> FeuRouge
    FeuRouge -->|Voitures attendent| FeuVert
...
L’objectif de PPO dans ce TP est d’apprendre à un agent à prendre de meilleures décisions (actions) dans un environnement, en mettant à jour sa politique (probabilités d’actions) et sa valeur des états de manière progressive et stable.
 ...
 ### 📁 Fichier : `TP_04.ipynb`
  #### 📊 Schéma du fonctionnement de PPO
```mermaid
graph TD;
    Start --> CollecteDonnees
    CollecteDonnees --> MiseAJourPolitique
    MiseAJourPolitique --> Verification
    Verification -->|Si amelioration| NouvellePolitique
    Verification -->|Sinon| AjustementParametres
    AjustementParametres --> MiseAJourPolitique
```
